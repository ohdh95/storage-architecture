...
static const char* get_zone_state_str(int state)
{
    switch (state) {
        case ZONE_EMPTY:
            return "EM";
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		case 'D':
			...
			for (i = 0; i < nzone; i++)
				printf("%d:%s,%d,%d ", lba_to_zone(lba) + i,
						get_zone_state_str(desc_buf[i].state), desc_buf[i].slba, desc_buf[i].wp);
			printf("]\n");
			free(desc_buf);
			break;
		case 'C': {
		...
		case 'T': {
			...
			printf("TL Open(%d)", zone);
			if (zns_tl_open(zone, valid_arr) == -1)
				printf(": failed");
			printf("\n");
			free(valid_arr);
			break;
